...
Premier_league(version1):
  I got this data set from 'https://www.kaggle.com/datasets/saife245/english-premier-league/data?select=final_dataset.csv'.This project just predicts the result of the   matches before the actual match.My target varible is FTR(FullTimeResult), this dataset was designed for sports betting so it had a lot of betting related      data so i dropped them all and other columns that were not required.After dropping all the columns my features were HomeTeam,AwayTeam,FTR(y).
